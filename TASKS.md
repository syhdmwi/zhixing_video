...
> **历史说明**：本轮执行时「羊毛毡定格动画风」确实作为第 6 个预设加入。后续 Round 4 删除了「3D科技讲解风」并重排编号，因此当前最终 `style-presets.md` 中该预设编号为 `### 5. 羊毛毡定格动画风`，当前枚举为 `1-5 + 0`。
...
> - 规范名 `omni`，**API model ID = `omni_flash`**。
...
- 图生视频模型表新增一行：`omni` / 用户可见=是 / 用途=图生视频，支持首尾帧(多图 `|` 分隔)与视频参考(v2v) / 脚本 `video_generation_router.py` / 别名收敛 `omni_flash`。
- veo 行补注：`veo_3_1-fast`/`-fast-fl`/`-4K` 为同 `/v1/videos` endpoint 的实现层变体。
- **验收**：omni 在表中且标用户可见；API model ID `omni_flash` 写明；endpoint 与鉴权准确。

### [x] R6.2 新增 omni-provider-notes.md
- 新建 `ai-video-generate-videos/references/omni-provider-notes.md`：endpoint、鉴权、参数(prompt/model=`omni_flash`/size WxH/input_reference 首尾帧或视频参考用 `|` 分隔/可选 remix_id)、异步 status+progress、轮询 `GET /v1/videos/{id}`、size 示例、无 duration；并写"与 grok 的区别：omni 支持首尾帧与视频参考"。
...
- `video_generation_router.py` 新增 provider `omni`，复用 `/v1/videos` create+poll（与 veo 同形）：
  ...
  - `default_model_for_provider("omni")` → `omni_flash`
...
- **验收**：`python3 -m py_compile` 通过；omni 队列项能构造出正确 payload(model=`omni_flash`)；grok 路径不受影响。
...
> `[REVIEW Round 6]` Codex 停。Claude 检查：MODELS.md 登记准确(omni_flash)、notes 不编造字段、router omni 分支正确且 py_compile 通过且不破坏 grok、用户可选 grok/omni 默认 grok、示例 JSON 合法、无断链。
>
> **Claude review 结论（2026-06-04）：全部通过，已打本地 commit 存档点。**
> - R6.1/R6.2：MODELS.md 与 omni-provider-notes.md 准确写明 model ID omni_flash、POST /v1/videos 创建 + GET /v1/videos/{id} 轮询、YIJIA_API_KEY、首尾帧(|)与视频参考(v2v)；无编造字段。
> - R6.3：router 新增 omni 分支复用 submit_veo/poll_veo(/v1/videos 同形)，credentials/default-model(omni_flash)/terminal/render-url 齐全；choose_auto_provider 仍 grok-first，omni 仅显式启用，grok 失败不回退 omni；py_compile 通过。
