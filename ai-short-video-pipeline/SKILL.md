...
如果当前运行环境是 `Codex`,并且用户选择的生图模型是 `GPT-Image-2`,默认优先使用 `Codex` 自带的生图能力完成图片生成。

执行优先级:

1. `Codex` 环境 + `GPT-Image-2` -> 优先使用 `Codex` 自带生图能力
2. 非 `Codex` 环境 + `GPT-Image-2` -> 使用外部脚本 / API 队列方式
3. `nanobanana-2` -> 继续按现有外部脚本 / API 方式执行

硬规则:

- 只要当前运行环境是 `Codex`,并且用户选择的是 `GPT-Image-2`,无论是单张测试、前 10 张测试批次、审一张改一张,还是大批量正式生图,都默认使用 `Codex` 自带生图能力
- 不要因为“批量更方便”就自动切到外部脚本
- 不要因为“需要队列”就自动切到外部脚本
- 只有在 `Codex` 自带生图能力当前不可用时,才允许回退到外部脚本 / API
