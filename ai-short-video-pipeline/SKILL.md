...
当前可用模板:
- `AI科普赛博明亮HUD风 请输入 1` - 科技/AI 科普,暗底霓虹,冷调未来感,纯画面无人物
- `手绘水彩教学风 请输入 2` - 温暖教学/入门科普,手绘水彩,蓝围巾小人讲解
- `3D科技讲解风 请输入 3` - 商业短视频科普,3D渲染+可替换主讲角色,轻赛博干净高级
- `自主设定风格请输入 0`
...
用户选择风格模板(输入 `1` 或 `2`)后,必须执行以下步骤,不是只记一个选择:

1. **加载模板**:从 `references/prompt-generation-rules.md` §3 风格参数表中加载对应预设的完整 6 组基因 + 负面约束。当前可选:「AI科普赛博明亮HUD风」「手绘水彩教学风」(color_palette、lighting_profile、texture_profile、composition_tendencies、camera_language、mood_keywords + era_spatial + post_processing)以及该预设的默认负面约束

2. **展示核确认**:用精简格式展示模板核心特征(风格名 + 色彩倾向 + 光影类型 + 情绪关键词),不要吐出完整 JSON。用户确认后锁定

3. **注入全局风格上下文**:确认后的 6 组基因 + 负面约束全局锁定。当前项目后续所有步骤(剧本解析后的提示词拼装、生图)都必须引用这个风格上下文,不得中间重新问「什么风格」
