...
如果当前运行环境是 `Codex`,并且用户选择的生图模型是 `GPT-Image-2`,默认先使用 `Codex` 自带的生图能力完成图片生成。

执行优先级:

1. `Codex` 环境 + `GPT-Image-2` -> 优先使用 `Codex` 自带生图能力
2. 非 `Codex` 环境 + `GPT-Image-2` -> 使用外部脚本 / API 队列方式
3. 任意环境 + `nanobanana-2` -> 继续按现有外部脚本 / API 方式执行
...
- 如果任务虽然超过 `10` 张,但仍处在风格测试或人物确认阶段,允许先用 `Codex` 生成 `1` 到 `3` 张样张确认,再切到外部脚本跑全量

推荐分工:

- `Codex` + `GPT-Image-2` 适合：
  - 单张测试
  - 审一张改一张
  - 前 10 张这类体验流程
- 外部脚本 / API 更适合：
...
- 在 `Codex` 中,如果本次任务总张数是 `1` 到 `10` 张,默认优先走内建生图
- 在 `Codex` 中,如果本次任务总张数是 `11` 到 `200` 张,默认改走外部脚本 / API
- 在 `Codex` 中,如果任务已经进入大批量正式生产,并且用户明确要批量、队列、日志或轮询能力,应优先切到外部脚本 / API
- 不要在没有说明原因的情况下,把 `Codex` 小批量测试自动切到外部脚本
- 不要在没有说明原因的情况下,把 `Codex` 的 `11` 张以上正式批量任务继续拖在内建生图里逐张处理
- 如果 `Codex` 自带生图能力当前不可用,才允许直接回退到外部脚本 / API
...
时长估算之后,先进入**剧本解析**和**空间锁定卡**(见 shot-planner Step 0 和 Step 0.5),再进入正式镜头拆分。剧本解析负责把文案拆成结构化要素(角色、场景、情绪线、色调线、关键动作),空间锁定卡负责固定每个场景的空间关系(环境锚点、光线方向、轴线管理)。

在正式提示词生成之前,必须先给每个分镜打 `frame_type` 标签,至少区分:

- `people_primary`：人物主镜头
- `people_with_scene`：人物参与镜头
- `scene_only`：纯场景镜头
- `concept_explainer`：概念解释/信息图镜头

`frame_type` 属于中间控制字段,作用是让后续的提示词数量配比和镜头节奏可以在“提示词生成前”被检查和修正,而不是等整批图片生成后再返工。

正式提示词输出前,必须先做一次镜头配比检查:

- 默认目标仍是 `scene_only` 约占至少三分之一
- 其余镜头以 `people_primary` 和 `people_with_scene` 为主
- `concept_explainer` 视项目性质归入“非纯人物解释镜头”,可参与平衡整体节奏,但不能全部替代纯场景镜头

如果配比明显失衡,优先动作是:

1. 修改对应镜头的 `frame_type`
2. 同步改写该镜头的 `visual_goal`
3. 再重新生成这些镜头的正式提示词

不要等整批正式图片生成完成后,才发现“人物太多”或“场景太多”再被动重跑。

这条规则的优先级低于剧本解析和空间锁定卡,也就是说:

- 先服从剧本解析决定“什么信息适合人物讲、什么信息适合场景/概念解释”
- 再服从空间锁定卡决定“这些镜头在什么空间里成立”
- 最后才用 `frame_type` 配比检查做整体平衡
