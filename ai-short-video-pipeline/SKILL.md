...
当前可用模板:
- `赛博朋克风格 请输入 1` - 偏动漫化和风格化表达,霓虹科技感强,可做夜景也可做高通透白天赛博场景
- `素描风格 请输入 2` - 铅笔线稿、黑白灰层次、画面简洁、结构感明确
- `自主设定风格请输入 0`
...
1. **加载模板**:从 `ai-video-image-prompts/references/style-presets.md` 中加载对应预设的完整风格上下文。当前可选:「赛博朋克风格」「素描风格」。加载内容不只是预设名,还包括 `style_block`、色彩倾向、光影倾向、材质倾向、环境倾向、构图倾向、负面约束建议,以及可用时的 `visual_style` 结构化字段
