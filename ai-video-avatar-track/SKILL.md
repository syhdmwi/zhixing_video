...
- 如果用户上传本地音频或本地视频，且已配置 TOS，默认先自动上传到 TOS，再把公网直链提交给数字人接口
- 如果用户直接提供公网可访问 URL，则直接提交给数字人接口
- 只有在 TOS 配置缺失且用户只提供本地文件时，才提示用户补充 TOS 配置或提供公网 URL
...
- `是否需要继续制作数字人视频轨？如果需要，请上传音频和正面人物视频，我会自动转存并提交数字人生成。`
