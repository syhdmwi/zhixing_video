...
对新手用户，默认按“向导式流程”推进。每次只问当前阶段必须回答的问题，并用 `当前进度` 告诉用户现在走到哪里。
...
8. 整批图片生成成功并展示后，必须询问用户是否满意、是否有需要修改的图片；如果有，让用户输入第几张和修改要求；如果不需要修改，输入 `2` 进入视频生成
...
- 如果重复主体属于抽象概念、系统组件、知识结构、流程节点、数据模块、界面机制等“抽象主体”，生成三视图之前必须先锁定统一的资产类型，不允许让模型自由决定有的变角色、有的变设备、有的变图标
- 抽象主体在未收到用户额外指定时，默认使用 `科技装置型` 资产路线，而不是角色化助手型或图标贴纸型
- 抽象主体三视图除了锁定资产类型，还必须锁定完成度类型：默认使用“商业科技插画设定图 / 科技讲解资产图”，禁止跑成写实工业产品渲染、硬件商品广告图、摄影棚产品展示图
- 如果项目已经生成出一批被用户认可的抽象主体三视图，后续同项目中的其它抽象主体必须以这批已确认结果作为风格锚点继续生成，保持同一套材质、光影、线条完成度和版式逻辑
...
- 如果用户提供的是真人头像、真人照片或只有正面的人物参考图，并且希望生成动漫/半 3D 二次元三视图，不要默认一步到位硬扩三视图。优先采用“两步法”：先生成正面半身动漫化角色定稿，让用户确认是否像本人；用户确认后，再用这张正面定稿扩展成白底三视图主体参考卡
- 进入真人参考图两步法时，应先告诉用户当前阶段：`主讲人参考图已收到。这类真人头像为了更像本人，不直接硬扩三视图。当前进度：1. 已锁定主讲人真人参考 2. 现在先生成正面动漫化定稿 3. 你确认像本人后，再扩展白底三视图`
- 两步法的第一步必须优先锁定“像不像本人”，不要同时追求复杂背景、剧情画面或多角度。提示词应强调脸型、五官比例、眼睛形状、眼距、鼻子、嘴唇、下巴、额头比例、发型和气质
- 两步法的第二步必须使用已确认的正面定稿作为角色锚点来扩白底三视图，不再直接依赖真人头像硬扩。三视图应保持同一角色、同一服装、同一发型、同一风格
...
- 正式整批图片生成成功后，不允许直接进入视频阶段。必须先展示全部图片，并固定询问：`是否满意，有无需要修改的图片？如果有请输入第几张，并把修改要求一起输入；如果不需要修改，输入 2 进入视频生成。`
- 如果用户要求修改某几张正式图片，只重生成对应图片，不要默认重跑整批
- 用户满意主体清单后，先生成主体三视图作为参考图
- 正式 30 张图应尽量以已确认主体图和三视图作为参考锚点
- 所有主体预览图默认使用白底
- 所有三视图参考图默认使用白底
- 抽象主体三视图默认使用白底或浅灰底的“设定图 / 资产图”版式，正面、侧面、背面同图展示，但整张图只能服务于一个主体
- 抽象主体三视图提示词里，默认加入：`统一商业科技插画资产风格`、`不要角色化吉祥物`、`不要Q版化`、`不要写实工业产品渲染`、`不要真实硬件商品摄影感`
- 正式分镜图里，只要画面包含人物，默认以半身照、半身构图、胸像构图为主；但不应全部锁死为半身图。为了避免构图疲劳，允许少量全身、远景、背影、侧影、环境人像穿插。只有当用户明确要求全程半身时，才强制全部使用半身构图。
...
如果用户输入 `1`、`2`、`3`、`4`、`5` 或 `6`，从 [references/style-presets.md](./references/style-presets.md) 加载对应预设的完整风格上下文。

对熟练用户，风格模版、图片比例和生图模型可以放在同一轮确认；对新手用户，优先拆成逐步确认。

如果用户没有明确选择图片比例，默认按 `16:9` 处理，不要因为缺这一个字段就停住整条图片流程。

图片阶段的前台固定选项块，默认按这个格式展示：
...
如果用户提供的是人物三视图，不要简单只取正面，也不要直接把三视图版式带入正式画面。应先判断这张三视图是否适合作为模型可读的主体参考卡；如果存在强背景色、动作遮挡、角度边界不清等问题，应提醒用户可能导致重影或多角度融合，并建议先整理成更干净的主体参考卡。

正式生图使用人物三视图或主体参考卡时，提示词必须明确：参考图只用于保持人物身份一致性，不是最终画面内容；不要复刻三视图版式；不要生成多角度展示页；不要把正面、侧面、背面融合到同一个人物身体里；最终画面只生成一个当前镜头中的人物。
...
如果用户要套用模板，优先先读取模板中的主讲人锚点、风格摘要、提示词规则和默认参数，再继续后续流程。

套用模板时，不要把旧文案里的重复主体自动带入新文案。新文案中的重复主体仍然需要重新识别、展示清单并让用户确认。
...
例如当用户选择 `AI科普赛博明亮HUD风` 时，不应只写一句“AI科普赛博明亮HUD风”，而应继承该预设里的完整风格块、色彩倾向、光影倾向、环境倾向和负面约束建议。
...
- 有人物的正式画面，默认以半身照、半身特写或胸像构图为主
- 不要整批都生成全身站立图，也不要整批都生成半身图
- 需要看清观点表达、表情、手势、讲解动作时，优先半身或胸像
- 需要表现空间关系、人物进入场景、对比规模、行动路径时，允许全身、远景、背影、侧影或环境人像
- 如果一批图里人物镜头较多，建议约 `70%` 使用半身/胸像/近景，约 `30%` 使用全身/远景/背影/环境人像
- 只有用户明确要求“全部半身”或“全部全身”时，才按用户要求统一构图
