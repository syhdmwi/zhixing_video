...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run a GPT-Image-2 image-generation batch from an approved queue file.")
...
def main() -> int:
    ...
    api_key = os.environ.get("GPT_IMAGE2_API_KEY") or os.environ.get("WUYINKEJI_API_KEY")
    if not api_key:
        print("GPT_IMAGE2_API_KEY or WUYINKEJI_API_KEY is not set", file=sys.stderr)
        return 2

    queue_file = Path(args.queue_file)
    queue = load_queue(queue_file)
    normalized_queue = [
        normalize_item(item, args.aspect_ratio, args.model)
        for item in queue
    ]
